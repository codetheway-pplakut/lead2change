...
namespace Lead2Change.Services.Students
{
    public class StudentService : _BaseService, IStudentService
    {
        ...
        public async Task<RegistrationViewModel> Register(RegistrationViewModel model)
        {
            var response = await _studentRepo.Create(new Student() { 
                Id = Guid.NewGuid(), //NOTE: This is an example only. The ID should be generated by the DB.
                FirstName = model.FirstName,
                LastName = model.LastName
            });

            //NOTE: We need to do error handling for all points of failure
            if (response.Message == StringConstants.ERROR)
            {
                await LogEvent(new AppEvent() { 
            ...
            //NOTE: Example
            //var goal = await _goalRepo.Create();
